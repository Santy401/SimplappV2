...
export function InventoryWidget({ title, items, className }: InventoryWidgetProps) {
  return (
      ...
      <div className="space-y-4 mt-2">
        {items.length === 0 ? (
          <div className="py-10 text-center">
            <p className="text-sm text-slate-400">Todo tu inventario está al día.</p>
          </div>
        ) : (
          items.map((item) => {
            ...
            return (
              <div key={item.id} className="group">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-bold text-slate-700 dark:text-slate-300 group-hover:text-purple-600 transition-colors">
                    {item.name}
                  </span>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={cn(
                      "text-xs font-black",
                      item.stock <= item.minStock ? "text-rose-600" : "text-slate-500"
                    )}>
                      {item.stock} unidades
                    </span>
                  </div>
